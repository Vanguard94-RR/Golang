...
// Another example of pointers usage in Go is to create a pointer to a new variable and modify
// its value through the pointer.
func createAndModifyPointer() {
	// Create a new variable using the new() function
	ptr := new(int)
	fmt.Println("Initial value at pointer:", *ptr) // Output: 0 (zero value for int)
	fmt.Println("Address of pointer:", ptr)

	// Modify the value through the pointer
	*ptr = 300
	fmt.Println("Modified value at pointer:", *ptr)

	// Create another pointer and assign it to the same address
	anotherPtr := ptr
	*anotherPtr = 400
	fmt.Println("Value modified through another pointer:", *ptr)
}
...
func main() {
	...
	*numPtr = 100
	...
	// Another example of pointers usage in Go is to create a pointer to a new variable and modify its value through the pointer.
	Tittle("Creating and Modifying Pointers")

	createAndModifyPointer()
}
